 ...
 *
 * Proxies requests to the Unsplash user collections endpoint.
 * Extracts the cover_photo from each collection and returns them in the same
 * { results, total_pages } shape as /api/images so the existing hook and
 * components can consume it without modification. Collections without a
...
async function GET(
  ...
  const apiUrl = `https://api.unsplash.com/users/${username}/collections?${apiParams.toString()}`;
  ...
  try {
    ...
    const collections = (await response.json()) as UnsplashCollection[];
